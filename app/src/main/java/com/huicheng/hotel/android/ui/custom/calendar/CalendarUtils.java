...
public class CalendarUtils {
    ...
    private static final String[] daysOfAlmanac = {"初一", "初二", "初三", "初四", "初五", "初六", "初七", "初八", "初九", "初十",
            "十一", "十二", "十三", "十四", "十五", "十六", "十七", "十八", "十九", "二十",
            "廿一", "廿二", "廿三", "廿四", "廿五", "廿六", "廿七", "廿八", "廿九", "三十"}; // 农历的天数
    private static final String[] weekStr = {"日", "一", "二", "三", "四", "五", "六"};
    private static final String[] monthStr = {"一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月", "十二月"};
    ...
    public String getChineseDay(int y, int m, int d) {
        ...
        if (sectionalTerm > 0 && sectionalTerm == gregorianDate) {
            return sectionalTermNames[gregorianMonth - 1];
        } else if (principleTerm > 0 && principleTerm == gregorianDate) {
            return principleTermNames[gregorianMonth - 1];
        } else {
            return daysOfAlmanac[chineseDate - 1];
        }
    }
...
//    }
    ...
    public static String getMonthStringByNum(int index) {
        return monthStr[index];
    }

    public static String getWeekStringByNum(int index) {
        return weekStr[index];
    }

    public static boolean isSortBeginLast(SimpleMonthAdapter.CalendarDay begin, SimpleMonthAdapter.CalendarDay last) {
        if (last.year > begin.year) {
            return true;
        } else if (last.year < begin.year) {
            return false;
        } else {
            if (last.month > begin.month) {
                return true;
            } else if (last.month < begin.month) {
                return false;
            } else {
                if (last.day > begin.day) {
                    return true;
                } else if (last.day < begin.day) {
                    return false;
                } else {
                    return false;
                }
