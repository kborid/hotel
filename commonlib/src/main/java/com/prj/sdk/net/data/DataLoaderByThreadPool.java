 ...
 * 数据异步加载
 */
public class DataLoaderByThreadPool {

    private final String TAG = getClass().getSimpleName();

    private static DataLoaderByThreadPool mInstance = null;
    private ExecutorService threadPools = null;
    private HashMap<String, Future> mFutures = null;
    ...
    private DataLoaderByThreadPool() {
    ...
    public static DataLoaderByThreadPool getInstance() {
        if (mInstance == null) {
            synchronized (DataLoaderByThreadPool.class) {
                if (mInstance == null) {
                    mInstance = new DataLoaderByThreadPool();
                }
            }
        }
        return mInstance;
    ...
    private void insertRequestQueue(String id, Runnable request) {
        Future future = threadPools.submit(request);
    ...
    public synchronized void clear(String id) {
        if (mFutures.containsKey(id)) {
            if (mFutures.get(id).isCancelled() || mFutures.get(id).isDone() || mFutures.get(id).cancel(true)) {
                mFutures.remove(id);
            }
        }
    }
